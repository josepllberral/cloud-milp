...
// TODO - Pre-emptive scheduling
//		- Llegir fitxer sencer i crear la Timetable
//		- Fer connexions preemptives
...
class QuickRepro
{
		...
		try
		{
			...
			while ((strLine = br.readLine()) != null)
			{
				if (strLine.contains("load from it:"))
				{
					...
					// Load for VM-C
					if (ListOfVMc.containsKey(new Integer(aux[2])))	LoadLocation = ListOfVMc.get(new Integer(aux[2]));
					else LoadLocation = new HashMap<String,Integer>();

					LoadLocation.put(aux[4],(new Integer((aux[10].split("\\."))[0])));
					ListOfVMc.put(new Integer(aux[2]),LoadLocation);
				}
	...
	private void schedule()
	{
		...
		{
			// TODO - Preemptive crap

			// Preveure on aniran les VMs al proper Schedule

			// mirar quines es poden migrar en 15 minuts

			// Obrir i tancar connexió per a preemtive sched
		}
	}
	...
	private void policyMS (Map <String,Integer> aggregatedVMTotal, Map <String,Integer> movementNumVM, Map <String,Integer> aggregatedTrafficTotal, Map <String,Integer> movementNumDB)
	{
		...
		for (Integer vmkey : ListOfVMs.keySet())
		{
			...
			for (String key : requiredPathVMBWD.keySet())
			{
				...
				if (auxbwd < reqbwd)
				{
					...
					move = (new Double(move*ratio)).intValue(); 		// Some VMs are behind
				}
		...
		for (String key : requiredPathDBBWD.keySet())
		{
			...
			if (auxbwd < reqbwd)
			{
				...
				// TODO - Data was not finished to transmit
			}				
		}
		...
		for (int i = 0; i < count; i++ )
		{
			...
			currentTS = simTS;
			boolean exists = connections.containsKey(kname+"-"+orig+"-"+dest);
			if (exists)
			{
				int bwd = 1; // FIXME - Reduced only to 1Gbps... must adjust to common traffic
				boolean done = negotiateBWD(kname,aux[0],aux[1],bwd,true);
	...
	private boolean negotiateTime (String keyname, String origin, String destination, BigInteger volume, long time)
	{
		...
		while (keepGoing)
		{
			long resaux = timedConn(keyname,origin,destination,volume,newTime);
			if (resaux > 0)
			{
				done = true;
				retval = true;
				...
				int id = retrieveTCID(keyname,origin,destination);
				System.out.println("* Negotiation Success " + origin + " -> " + destination + " id: " + id + " bwd: " + bwd + " Gbps; Time: " + newTime + " seconds");
			}		
			else if (resaux <= 0)
			{
				newTime = -1 * resaux;						// Proposed new Time -> Game Over
				if (newTime < time * 4) keepGoing = false;	// Game Over, we keep old bwd FIXME - Stablish limit.
	...
	private long timedConn(String keyname, String origin, String destination, BigInteger volume, long time)
	{
		...
		else if (reply.contains("<status>KO</status>"))
		{
			...
			matcher.find();
			retval = -1 * (new Long(matcher.group(1))).longValue();	// Proposed new time
		}
		return retval;
	}
